...
# TODO: refactor scheduler to have a minimum learning rate when doing restarts
...
class LRScheduler:
    ...
    def step_lr(self, epoch_exact):
        ...
        if epochs_into_interval < self.restart_interval: # if we are within a restart interval
            # self.optimizer.param_groups[0]['lr'] = self.restart_fn(epochs_into_interval) - TODO: new way to overload tensors
            self.optimizer.param_groups[0]['lr'] = self.restart_fn(epochs_into_interval).clone().detach()
