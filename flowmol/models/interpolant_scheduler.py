...
class InterpolantScheduler(nn.Module):
    ...
    def __init__(self, canonical_feat_order: List[str], schedule_type: Union[str, Dict[str, str]] = 'cosine', cosine_params: dict = {}):
        super().__init__()
        ...
        if 'cosine' in self.schedule_types:
            for feat in cosine_params:
                # cosine_params[feat] = torch.tensor(cosine_params[feat]).unsqueeze(0) # TODO: new way of overloading tensors
                cosine_params[feat] = cosine_params[feat].unsqueeze(0).clone().detach()
