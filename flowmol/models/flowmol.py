...
class FlowMol(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 parameterization: str = 'endpoint', # how to parameterize the flow-matching objective, can be 'endpoint', 'vector-field', 'dirichlet', or 'ctmc'
        ...
        if self.parameterization not in ['endpoint', 'vector-field', 'dirichlet', 'ctmc']:
            raise ValueError(f'parameterization must be one of "endpoint", "vector-field", "dirichlet", or "ctmc", but got {self.parameterization}')
        ...
        # align the validation losses with the corresponding training epoch value on W&B
    ...
    def forward(self, g: dgl.DGLGraph):
        ...
        # it is necessary to do this here (as opposed to in __init__) because
        ...
        # sample time-points for each molecule in the batch
    ...
    def sample_random_sizes(self, n_molecules: int, device="cuda:0",
        ...
        if min_num_atoms is None and max_num_atoms is None:
    ...
    def sample(self, 
        ...
        if keep_intermediate_graphs:
            g, intermediate_graphs = itg_result
