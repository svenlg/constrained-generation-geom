...
class CTMCVectorField(EndpointVectorField):
    # uses Continuous-Time Markov Chain (CTMC) to model the flow of categorical features (atom type, charge, bond order)
    ...
    # our conditional trajectories interpolate along a progress coordinate alpha_t, which is a function of time t
    ...
    def sample_conditional_path(self, g, t, node_batch_idx, edge_batch_idx, upper_edge_mask):
        ...
        for feat, feat_idx in zip(['a', 'c'], [1,2]):
            ...
            # so here we convert to token indices by argmaxing, and then one-hot encode again but with mask token
    ...
    def integrate(
        ...
        **kwargs
        ...
        if tspan is None:
            t = torch.linspace(0.0, 1.0, n_timesteps, device=g.device)
        else:
            t = tspan
        ...
        # if visualizing the trajectory, create a data-structure to store the trajectory
        if visualize:
            traj_frames = {}
            for feat in self.canonical_feat_order:
                if feat == "e":
                ...
                init_frame = data_src[f'{feat}_0'].detach()
        ...
        for s_idx in range(1,t.shape[0]):
            ...
            if visualize:
                for feat in self.canonical_feat_order:
                    ...
                    frame = g_data_src[f'{feat}_t'].detach()
                    if feat == 'e':
                        split_sizes = g.batch_num_edges()
                    else:
                        split_sizes = g.batch_num_nodes()
                    ...
                    ep_frame = g_data_src[f'{feat}_1_pred'].detach()
    ...
    def step(self, g: dgl.DGLGraph, s_i: torch.Tensor, t_i: torch.Tensor,
        ...
        # General step for positions
        # s = sigma, a = alpha, b = beta, (a = 1-b) _d = _dot
        # n = eta = b * (a_d/a * b - b_d)) = (if b = 1-a) = 2 * a_d * (1-a) / a
        # x_t+1 = x_t + dt (vf + (s^2 / n) * (vf - alpha_dot / alpha x_t) + dt * s * noise
        # if s = 0 -> Euler step
        # if s = sqrt(2n) -> Memoryless step
        ...
        # x_t+1 = x_t + dt (2 * vf - alpha_dot / alpha x_t) + dt * sigma * noise
            ...
            sigma = torch.sqrt(2 * alpha_t_prime_i[0] * (1 - alpha_t_i[0]) / (alpha_t_i[0] + dt))
            eps_pred = 2 * vf - alpha_t_prime_i[0]/(alpha_t_i[0]+dt) * x_t
            g.ndata['x_t'] = x_t + dt * eps_pred + torch.sqrt(dt) * sigma * torch.randn_like(x_t)
        ...
        for feat_idx, feat in enumerate(self.canonical_feat_order):
            ...
            p_s_1 = F.softmax(torch.log(p_s_1)/temperature + 10e-5, dim=-1) # log probabilities
