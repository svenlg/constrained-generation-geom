...
# def compute_property_stats(molecules, property, device, model=None):
#     properties = []
#     if molecules.ndata['a_t'].shape[1] == 6:
#         atom_type_map = atom_type_list_qm9
#     elif molecules.ndata['a_t'].shape[1] == 11:
#         atom_type_map = atom_type_list_geom
#     else:
#         raise ValueError(
#             f"Unsupported atom type shape: {molecules.ndata['a_t'].shape[1]}. "
#             "Expected 6 for QM9 or 11 for GEOM. (Note CTMC uses 6/11 atom types)"
#         )
#     for mol in dgl.unbatch(molecules):
#         if property in ["energy", "dipole"]:
#             val, _ = compute(
#                 molecule = mol,
#                 atom_type_map = atom_type_map,
#                 property = property, 
...
#         elif property == "sascore":
#             assert model is not None, "Model must be provided for SAScore calculation"
#             val, _ = sa_score(mol, model, 1.0, device, return_grad=False)
#             properties.append(val)
#         else:
#             raise ValueError(f"Unknown property: {property}")
#     properties = torch.stack(properties)
#     finite_mask = torch.isfinite(properties)
#     properties_finite = properties[finite_mask].detach().cpu()
#     properties_mean = properties_finite.mean().detach().item()
#     properties_median = properties_finite.median().detach().item()
#     properties_std = properties_finite.std().detach().item()
#     num_invalid = len(properties) - len(properties_finite)
#     return {
#         "mean": properties_mean,
#         "std": properties_std,
#         "num_invalid": num_invalid,
#         "median": properties_median,
#         "all_finite": properties_finite,
#     }
...
#             gradients.append(grad)
#         elif property == "const":
#             # For constant property, we return zero value and gradient
#             val = torch.tensor(0.0, device=device)
#             properties.append(val)
#             grad = torch.zeros_like(mol.ndata['x_t'], device=device)
#             gradients.append(grad)
#         elif property == "sascore":
#             assert model is not None, "Model must be provided for SAScore calculation"
#             val, grad = sa_score(
