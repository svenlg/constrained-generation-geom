        ...
        with torch.enable_grad():
            ...
            if self.normalize:
                ...
                if isinstance(x, dgl.DGLGraph):
                    ...
                    grad.set_batch_num_nodes(x.batch_num_nodes())
                    grad.set_batch_num_edges(x.batch_num_edges())
                    grad.ndata['x_t'] = combined[0].clone().detach().requires_grad_(False)
                    grad.ndata['a_t'] = combined[1].clone().detach().requires_grad_(False)
                    grad.ndata['c_t'] = combined[2].clone().detach().requires_grad_(False)
                    grad.edata['e_t'] = combined[3].clone().detach().requires_grad_(False)
                    grad.edata['ue_mask'] = x.edata['ue_mask'].detach().clone()
                    full_grads = [combined[0], combined[1], combined[2], combined[3]]
                else:
                    grad = combined[0].clone().detach().requires_grad_(False)
                    full_grads = [combined[0]]
